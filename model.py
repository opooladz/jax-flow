...
from utils.spectral_optimizer import spectral_init
from math_utils import get_2d_sincos_pos_embed, modulate

class TrainConfig:
    def __init__(self, use_spectral, dtype, scale_factors):
        self.dtype = dtype
    ...
    def kern_init(self, name='default', zero=False):
        if not self.use_spectral:
            ...
            return spectral_init(init_scale=(0 if zero else 1), lr_scale=self.scale_factors[name])
...
class TimestepEmbedder(nn.Module):
    ...
    def __call__(self, t):
        ...
        x = nn.Dense(self.hidden_size, kernel_init=self.tc.kern_init('time'), 
                     bias_init=self.tc.kern_init('time_bias'), dtype=self.tc.dtype)(x)
        x = nn.silu(x)
        x = nn.Dense(self.hidden_size, **self.tc.default_config())(x)
        return x
...
class LabelEmbedder(nn.Module):
    ...
    def __call__(self, labels):
        ...
        embeddings = embedding_table(labels) * self.tc.scale_factors['label']
        return embeddings
...
class FinalLayer(nn.Module):
    ...
    def __call__(self, x, c):
        ...
        x = nn.Dense(self.patch_size * self.patch_size * self.out_channels, 
                     kernel_init=self.tc.kern_init('final'), 
                     bias_init=self.tc.kern_init('final_bias'), dtype=self.tc.dtype)(x)
        return x
...
class DiT(nn.Module):
    ...
    lr_scale_time: float = 1.0
    lr_scale_bias: float = 1.0
    ...
    def __call__(self, x, t, y, train=False, force_drop_ids=None, return_activations=False):
        ...
        scale_factors = {
            'default': 1.0,
            'bias': self.lr_scale_bias,
            'time': self.lr_scale_time,
            'time_bias': self.lr_scale_time * self.lr_scale_bias,
            'label': self.lr_scale_embed,
            'patch': self.lr_scale_patch,
            'patch_bias': self.lr_scale_patch * self.lr_scale_bias,
            'final': self.lr_scale_final,
            'final_bias': self.lr_scale_final * self.lr_scale_bias
        }
        tc = TrainConfig(use_spectral=self.use_spectral_norm, dtype=self.dtype, scale_factors=scale_factors)
        ...
        x = FinalLayer(self.patch_size, out_channels, self.hidden_size, tc)(x, c) # (B, num_patches, p*p*c)
