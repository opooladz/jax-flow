...
FLAGS = flags.FLAGS
flags.DEFINE_string('dataset_name', 'tiny-imagenet', 'Environment name.')
...
def main(_):
    ...
    def get_dataset(is_train):
        ...
        if FLAGS.dataset_name in ['tiny-imagenet', 'cifar10', 'cifar100']:
            # Determine image size based on dataset
            if FLAGS.dataset_name == 'tiny-imagenet':
                image_size = 64
                num_classes = 200
            ...
            # Update model config
            FLAGS.model['num_classes'] = num_classes
            
            # Create HF dataloader
            # CIFAR datasets use 'test' instead of 'valid'
            if FLAGS.dataset_name in ['cifar10', 'cifar100']:
                split = 'train' if (is_train or FLAGS.debug_overfit) else 'test'
            else:
                split = 'train' if (is_train or FLAGS.debug_overfit) else 'valid'
            hf_loader = create_hf_dataloader(
            ...
            # If debug overfit, take only a few batches
            if FLAGS.debug_overfit:
                # Create a limited iterator for overfitting
                def limited_iterator():
                    batch_list = []
                    for i, batch in enumerate(hf_loader.get_batch_iterator()):
                        batch_list.append(batch)
                        if i >= 7:  # Take 8 batches
                            break
                ...
                dataset = limited_iterator()
            else:
                # Convert to tuple format expected by rest of code
                def convert_iterator():
                    for batch in hf_loader.create_infinite_iterator():
                        yield batch['image'], batch['label']
                dataset = convert_iterator()
            
            return dataset
            
        # Original TensorFlow datasets code
        elif HAS_TENSORFLOW and 'imagenet' in FLAGS.dataset_name:
    ...
    try:
        ...
        if memory_stats:
            print("Total Memory on device:", float(memory_stats['bytes_limit']) / 1024**3, "GB")
    except:
        pass  # CPU devices don't have memory stats
    ...
    try:
        ...
        imagenet_labels = [f"class_{i}" for i in range(FLAGS.model['num_classes'])]
    if FLAGS.model.use_stable_vae:
