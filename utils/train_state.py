...
class TrainStateEps(flax.struct.PyTreeNode):
    rng: Any
    step: int
    ...
    def create(cls, model_def, params, rng, tx=None, opt_state=None, **kwargs):
        ...
        return cls(
            rng=rng, step=1, apply_fn=model_def.apply, model_def=model_def, params=params, params_eps=params,
    ...
    def call_model_eps(self, *args, params=None, method=None, **kwargs):
        return self.__call__(*args, params=self.params_eps, method=method, **kwargs)

    # Tau should be close to 1, e.g. 0.999.
    def update_eps(self, tau):
        new_params_eps = jax.tree_map(
            lambda p, tp: p * (1-tau) + tp * tau, self.params, self.params_eps
        )
        return self.replace(params_eps=new_params_eps)

    # For pickling.
    def save(self):
        return {
            'params': self.params,
            'params_eps': self.params_eps,
...
# def target_update(model_params, target_model, tau):
...
#     @classmethod
#     def create(cls, model_def: nn.Module, params, tx=None, opt_state=None, **kwargs):
#         if tx is not None and opt_state is None:
#             opt_state = tx.init(params)
